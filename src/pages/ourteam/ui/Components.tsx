  ...
  return (
    <div ref={containerRef} className="relative w-full" id={title}>
      <section className="flex flex-col overflow-hidden">
        <div className="flex h-screen w-full flex-row items-center justify-between sm:gap-20">
          <div className="flex flex-none flex-col gap-2 sm:gap-9">
            <motion.h2
              className="text-brand-primary-cta text-[25px]/[25px] font-bold sm:text-[100px]/[100px]"
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={isSectionInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 10 }} transition={{ duration: 0.5, delay: 0.3 }}>
              <AnimationButton href="joinus" className="px-3.5 py-1.5">
                <span className="text-custom-gray-100 text-[10px] sm:text-[28px]">바로 지원하러 가기</span>
              </AnimationButton>
            </motion.div>
          </div>
        ...
        <div className="relative flex h-[50vh] items-end">
          <div className="from-background-surface/30 sm:from-background-surface/60 to-background-surface/0 pointer-events-none absolute inset-0 z-50 bg-linear-to-t to-47%" />
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            animate={isBottomVisible ? { opacity: 1, y: 0 } : { opacity: 0, y: -60 }}
            transition={{ duration: 0.6, delay: 0.2 }}
            className="bg-surface-elevated flex w-full rounded-sm px-3 py-2.5 sm:rounded-2xl sm:px-12 sm:py-10"
          >
            <div className="z-10 sm:space-y-20 sm:pt-10">
              <motion.div
                style={{
              ...
              >
                <Hello className="h-2 w-2 sm:h-8 sm:w-8" />
              </motion.div>
      ...
      <div ref={bottomCardRef} className="relative flex h-[50vh] items-start gap-2.5 pt-2.5 sm:gap-12 sm:pt-12">
        <motion.div
          initial={{ opacity: 0, x: -20 }}
          animate={isBottomVisible ? { opacity: 1, x: 0 } : { opacity: 0, x: -20 }}
          transition={{ duration: 0.6, delay: 0.4 }}
          className="bg-surface-elevated flex h-20 flex-1 flex-col justify-end rounded-sm px-3 py-2.5 sm:h-79 sm:rounded-2xl sm:px-12 sm:py-10"
        >
          <p className="text-custom-gray-100 text-[12px]/4 font-medium whitespace-pre-line sm:text-[44px]/15">{description}</p>
        </motion.div>
        ...
        <motion.div
          ...
          animate={isBottomVisible ? { opacity: 1, x: 0 } : { opacity: 0, x: 20 }}
          transition={{ duration: 0.6, delay: 0.4 }}
          className="bg-surface-elevated flex h-20 flex-col rounded-sm px-2 py-2 sm:h-79 sm:rounded-2xl sm:px-8 sm:py-9.5"
        >
          <p className="text-brand-secondary-light text-[5px]/1.5 sm:text-[20px]/6">이런 기술 스택을 주로 이용합니다.</p>
