...
import { User, Mail, Phone, Hash, ShieldCheck, Check, Lock } from 'lucide-react';
      ...
      <div className="w-full max-w-105">
        <AnimatePresence mode="wait">
          {resetPasswordMutation.isSuccess ? (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center rounded-2xl p-10 text-center">
              <div className="bg-brand-primary-cta/10 mb-6 flex h-16 w-16 items-center justify-center rounded-full">
                <Check className="text-brand-primary-cta h-10 w-10" />
              </div>
            ...
            <motion.form onSubmit={onSubmit} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
              ...
              <div className="space-y-3">
                <Input icon={User} placeholder="아이디(UID)" value={formData.uid} onChange={(e) => handleInputChange('uid', e.target.value)} required />
                <Input icon={ShieldCheck} placeholder="이름" value={formData.name} onChange={(e) => handleInputChange('name', e.target.value)} required />
              </div>
              ...
              >
                {resetPasswordMutation.isPending ? '처리 중...' : '비밀번호 재설정 완료'}
              </button>
            </motion.form>
          )}
        </AnimatePresence>
